...
function basePlugins(): PluginsMap {
  return {
    hacker_news: { enabled: true, inline: true, hover: false, palette: true },
    github: { enabled: true, hover: false, palette: true },
    youtube: { enabled: false, hover: true, palette: true },
...
describe('setPluginField', () => {
  it('flips hacker_news.enabled and leaves inline/hover on that source untouched', () => {
    ...
    expect(next).toEqual({
      hacker_news: { enabled: false, inline: true, hover: false, palette: true },
      github: { enabled: true, hover: false, palette: true },
      youtube: { enabled: false, hover: true, palette: true },
      twitter: { enabled: true, inline: false, hover: false, palette: true },
    });
  ...
  it('re-enabling a disabled source restores its prior inline/hover choices rather than resetting them', () => {
    ...
    expect(disabled.hacker_news).toEqual({
      enabled: false,
      inline: true,
      hover: false,
      palette: true,
    ...
    expect(reenabled.hacker_news).toEqual({
      enabled: true,
      inline: true,
      hover: false,
      palette: true,
