...
**Scales (in `tokens.css`).** A short type scale `--text-xs…xl` (0.72–1.15rem, 6 steps — collapsed from 22 ad-hoc sizes); a 4px spacing scale `--s-0-5…--s10`; tracking `--tracking-tight -0.01em` (headlines + wordmark) / `--tracking-label 0.04em`; line-height `--lh-tight/snug/body`; elevation `--elev-1/2/3` + `--scrim` (tuned per theme — dark uses a blacker shadow since a warm shadow is invisible on the near-black ground); and `--row-inset` (42px) — the favicon-column grid the day label, row title, HN line, and note all align to. Numbers use `tabular-nums` app-wide.
