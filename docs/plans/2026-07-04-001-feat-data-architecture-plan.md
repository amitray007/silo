...
- **Concurrency + restore guard (from U2 review):** the partial-unique index is the dedup backstop, so `createLink`'s read-then-write has a TOCTOU window — catch a `23505` on insert and fall back to merge rather than surfacing the error. Likewise `restore` (clearing `deleted_at`) can hit `23505` if a fresh live row was saved for that canonical_url while the original sat in trash; `restore` must detect the existing live row and refuse-with-message or merge-into-it, not let the raw error bubble up.
